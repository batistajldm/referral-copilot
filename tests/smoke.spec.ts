...
test('smoke test - app loads and shows the search form', async ({ page }) => {
  await page.goto('/');

  await expect(page.getByRole('heading', { name: 'Referral Copilot' })).toBeVisible();
  await expect(page.getByText('Find a facility')).toBeVisible();
  await expect(page.getByPlaceholder('e.g. oncology, ICU, maternity')).toBeVisible();
  await expect(page.getByRole('button', { name: 'Search' })).toBeVisible();
});

test('smoke test - searching returns evidence-ranked facilities', async ({ page }) => {
  await page.goto('/');

  await page.getByPlaceholder('e.g. oncology, ICU, maternity').fill('oncology');
  await page.getByRole('button', { name: 'Search' }).click();

  // Results header appears once the query resolves.
  await expect(page.getByText(/facilities ranked by evidence strength/i)).toBeVisible({
    timeout: 30000,
  });
